            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                            ListElement { version: "v1.7.46"; title: "Audio Auto-Switch: Multi-Device Support"; desc: "Fixed: users with two audio sources can now switch freely without the script overriding their choice"; icon: "󰕾"; clr: "green"; detail: "FIXED: audio_autoswitch.sh reacted to EVERY pactl event — including user-initiated default-sink changes. If you manually switched from USB headset to Bluetooth headphones, any subsequent event (volume change, app opening) flipped back to the first external device. CHANGED: Now only reacts to sink 'new' (device plugged) and 'remove' (device unplugged) events. Sink 'change' events (which include user manual switches) are ignored. Users with two or more external audio devices can now freely switch between them. ADDED: Startup check — if an external device is already connected when the script starts, it auto-switches immediately. ADDED: Helper functions for cleaner code. Telemetry prompt moved to beginning of install with other config prompts. Audio autoswitch service now runs unconditionally (not tied to telemetry opt-in). FIXED: Opting out of telemetry now also removes old scripts and timers from previous installs — stopping all existing telemetry systemd timers, deleting script files, cleaning up the hidden directory." }
